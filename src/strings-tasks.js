...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    sum += str.charCodeAt(str.indexOf(str[i]));
  }
  return sum;
}
...
function isPalindrome(str) {
  ...
  const regexp = /(!|\?|[ \t]|,)/g;
  return (
    str.toLowerCase().replaceAll(regexp, '') ===
    revStr.toLowerCase().replaceAll(regexp, '')
  );
}
...
function invertCase(str) {
  ...
  str.split('').forEach((a) => {
    if (a === a.toLowerCase()) {
      newStr.push(a.toUpperCase());
    } else {
      newStr.push(a.toLowerCase());
    }
  });
  return newStr.join('');
}
...
function encodeToRot13(str) {
  const str13 = str;
  const arr = [];
  str13.split('').forEach((i) => {
    if (i !== ' ' && i !== '!' && i !== '?') {
      if (i !== i.toUpperCase()) {
        arr.push(
          String.fromCharCode(
            'a'.charCodeAt() + ((i.charCodeAt() - 'a'.charCodeAt() + 13) % 26)
          )
        );
      } else {
        arr.push(
...
function getCardId(value) {
  ...
  const arr2 = ['♣', '♦', '♥', '♠'];
  return (
    arr1.indexOf(value[0]) +
    arr2.indexOf(value.length === 2 ? value[1] : value[2]) * 13
  );
}
